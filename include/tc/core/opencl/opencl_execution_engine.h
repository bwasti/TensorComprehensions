...
class OpenCLExecutionEngine : public ExecutionEngine {
 public:
  struct OpenCLExecutorInfo : public ExecutionEngine::ExecutorInfo {
    OpenCLExecutorInfo(
        std::string id,
        ...
        : ExecutionEngine::ExecutorInfo(
              ...
              handle) {
      exec =
          std::unique_ptr<OpenCLTcExecutor>(new OpenCLTcExecutor(tc, inputsInfo));
    }

    void clear() {
      // TODO
      //static_cast<OpenCLTcExecutor&>(*exec).clearRTC();
    }
  };

  OpenCLExecutionEngine() = default;
  ...
  Duration run(
    ...
    return run(handle, inputs, outputs, profile, [](const OpenCLExecutorInfo*) {
      return false;
    });
  }
  Duration run(
