...
Duration OpenCLExecutionEngine::run(
    size_t handle,
    const std::vector<const DLTensor*>& inputs,
    const std::vector<DLTensor*>& outputs,
    bool profile,
    std::function<bool(const OpenCLExecutorInfo*)> pruningFunction) {
  ...
  if (p.get()) {
    auto& execInfo = static_cast<OpenCLExecutorInfo&>(*p);
    auto exec = static_cast<OpenCLTcExecutor*>(execInfo.exec.get());
    ...
    try {
      ...
      LOG(ERROR) << "output length " << outputs.size();
...
// Steal ExecutorInfo and give it back under lock
// Run outside of lock on owning ExecutorInfo.
void OpenCLExecutionEngine::uncheckedRun(
    size_t handle,
  ...
  // exit.
  if (p.get()) {
    auto exec = static_cast<OpenCLTcExecutor*>(p->exec.get());
    CHECK(exec->hasRTCFun());
    try {
      // Must catch and swap to avoid exception in destructor!
...
// lock.
void OpenCLExecutionEngine::clear(size_t handle) {
  std::lock_guard<std::mutex> lg(executorInfoMutex);
  auto executor = static_cast<OpenCLExecutorInfo*>(executors_[handle].get());
  executor->clear();
  executors_[handle] = std::unique_ptr<ExecutionEngine::ExecutorInfo>(nullptr);
}

size_t OpenCLExecutionEngine::getHandle(
...
OpenCLExecutionEngine::makeExecutorInfo(
    const std::string& name,
    const std::vector<const DLTensor*>& inputsInfo,
    const MappingOptions& options) {
  CHECK_EQ(tcNameMap_.count(name), 1)
      << "TC function " << name << " not defined";
  return std::unique_ptr<ExecutionEngine::ExecutorInfo>(new OpenCLExecutorInfo(
      name,
      inputsInfo,
      options,
      tcNameMap_.at(name),
...
size_t OpenCLExecutionEngine::compile(
    const std::string& name,
    const std::vector<const DLTensor*>& inputs,
    const MappingOptions& options) {
